...
export default function ShoreCards() {
  return (
    <section className="container-page pb-6 pt-3 sm:pb-12 sm:pt-6">
      ...
      <div className="grid grid-cols-2 gap-2.5 md:gap-5">
        {SHORES.map((s) => (
          <Link
            key={s.title}
            href={s.href}
            className="group relative h-[205px] overflow-hidden rounded-2xl shadow-card md:h-64"
          >
            <SafeImage
              src={s.img}
              alt={s.alt}
            ...
            <span
              className={`absolute left-3.5 top-3.5 grid h-9 w-9 place-items-center rounded-full shadow-soft md:hidden ${s.iconBg}`}
            >
              <s.icon className="h-5 w-5" />
            </span>

            <div className="absolute inset-0 flex flex-col justify-end p-3 text-white md:p-6">
              <h3 className="text-base font-extrabold leading-tight md:text-2xl">
                {s.title}
              </h3>
              <p className="mt-0.5 line-clamp-2 text-[11px] leading-snug text-white/90 md:mt-1 md:text-sm">
                {s.desc}
              </p>
              <div className="mt-1.5 flex flex-wrap gap-1 md:mt-2.5 md:gap-1.5">
                {s.chips.map((c) => (
                  <span
                    key={c}
                    className="rounded-full bg-white/18 px-1.5 py-0.5 text-[9px] font-medium backdrop-blur-sm md:px-2.5 md:py-1 md:text-xs"
                  >
                    {c}
                  </span>
                ))}
              </div>
              <span className="mt-2 inline-flex w-fit items-center gap-1 rounded-lg bg-white/95 px-2.5 py-1.5 text-[11px] font-semibold text-ink transition group-hover:gap-2 md:mt-3.5 md:gap-1.5 md:rounded-xl md:px-3.5 md:py-2 md:text-sm">
